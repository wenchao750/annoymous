...
MODEL_PATH = os.path.join(BASE_DIR, "GeometryBridge-100")
...
# 是否打印每个 query 的 Top-10 检索结果
PRINT_TOP10_RESULTS = True
...
def main():
    ...
    print(f"Filtered queries for retrieval: {len(filtered_query_ids)}")
    ...
    print("Encoding filtered queries...")
    ...
    print("Retrieving Top-10 documents and calculating metrics...")
    ...
    for q_idx, qid in enumerate(filtered_query_ids):
        ...
        if PRINT_TOP10_RESULTS:
            print("\n" + "=" * 80)
            print(f"query_id: {qid}")
            print("-" * 80)
            print(f"{'rank':<6}{'document_id':<30}{'similarity':<16}{'label_score'}")
            print("-" * 80)

        for rank, idx in enumerate(top_results.indices.tolist(), start=1):
            doc_id = doc_ids[idx]
            similarity_score = float(scores[idx].detach().cpu().item())
            label_score = label_dict.get((qid, doc_id), 0)
